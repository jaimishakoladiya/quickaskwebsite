...
function RatingBox(props) {
  ...
  async function postreview() {
    const name =
      type === "admin"
        ? `${user.data.admin.firstname} ${user.data.admin.lastname}`
      ...
      <div style={{display:'flex',marginLeft:"650px"}}>
        {props.data ? (
          <PlayCircleOutlineIcon
            ...
            style={{ width: "43px", height: "43px", color: "darkcyan" }}
          />
        ) : null}
        <StarIcon style={{ color: "#CCCC00" }} />
        <h3>{props.rate}</h3>
      </div>
      <Dialog
        open={open}
        ...
        <DialogContent>
          <DialogContentText>
            <video
              ...
              src={`http://localhost:2002${props.path}`}
              ref={cameraScreen}
              id="gum"
              playsInline
              autoPlay
